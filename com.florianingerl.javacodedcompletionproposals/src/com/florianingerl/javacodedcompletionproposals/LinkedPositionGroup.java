 ...
 * <p>
 * All linked positions in a group should have the same content. Before 3.5.400,
 * this was enforced. Now, if one position of a mixed group gets edited, the
 * content of all other positions is replaced by the edited position's content.
 * </p>
 * <p>
 * Normally, new positions are given a {@link LinkedPosition#getSequenceNumber()
 * sequence number} which can be used by clients, e.g. in the UI as tab stop
 * weight. If {@link #NO_STOP} is used as weight, a position will not be
 * visited. If {@link #NO_STOP} is used for all positions, the first position in
 * a document is taken as the only stop as to comply with the behavior of the
 * old linked position infrastructure.
 * </p>
 * <p>
...
public class LinkedPositionGroup {
	...
	/**
	 * {@link LinkedPosition#getSequenceNumber() Sequence number} constant
	 * declaring that a position should not be stopped by.
	 */
	...
	/**
	 * <code>true</code> iff not all positions contain the same content. In that
	 * case, the contents of the last edited position will replace the contents
	 * of all other linked positions.
	 */
	private boolean fMustEnforceEqualContents = false;

	private List<LinkedPositionGroup> dependentGroups = new LinkedList<LinkedPositionGroup>();
	private List<LinkedPositionGroup> dependencyGroups;
	 ...
	 * already in this group. All positions added must be valid and disjoint;
	 ...
	 * </p>
	 * <p>
	 * Once a group has been added to a <code>LinkedModeModel</code>, it becomes
	 * <em>sealed</em> and no positions may be added any more.
	 * </p>
	 *
	 * @param position
	 *            the position to add
	 * @throws BadLocationException
	 *             if the position is invalid or conflicts with other positions
	 *             in the group
	...
	/**
	 * Checks whether all positions contain the same string as the given
	 * position. If not, then {@link #fMustEnforceEqualContents} is set to
	 * <code>true</code>.
	 *
	 * @param position
	 ...
	 *            the position to check
	 * @throws BadLocationException
	 *             if the disjointness check fails
	 */
	 ...
	 * @param event
	 *            the event
	 * @return <code>true</code> if <code>position</code> and <code>event</code>
	...
	Map<IDocument, TextEdit> handleEvent(DocumentEvent event) {
		...
		if (fLastPosition != null) {
			...
			try {
				...
				// that the changes technically originate from a parent
				// environment
				// if this happens, post notification changes are not accepted
				// anyway and
				// we can simply return null - any changes will be undone by the
				// undo
				// manager
				return null;
			}
	...
	private void updateDependents(Map<IDocument, List<ReplaceEdit>> map) {
		for (LinkedPositionGroup dependentGroup : dependentGroups) {
			dependentGroup.dependenciesChanged(map);
		}
	}

	private void dependenciesChanged(Map<IDocument, List<ReplaceEdit>> map) {

		String[] parameters = new String[dependencyGroups.size()];
		int i = 0;
		for (LinkedPositionGroup dependency : dependencyGroups) {
			parameters[i++] = dependency.getCurrentValue();
		}

		String s = null;
		try {
			s = (String) updateMethod.invoke(null, parameters);
		...
		for (LinkedPosition p : fPositions) {
			...
			edits.add(new ReplaceEdit(p.getOffset(), p.getLength(), s));
		}

	}

	private String getCurrentValue() {
		LinkedPosition p = fLastPosition;
		if (p == null) {
			p = fPositions.get(0);
		}
		try {
	...
	/**
	 * Sets the model of this group. Once a model has been set, no more
	 * positions can be added and the model cannot be changed.
	 */
	void seal() {
	 ...
	 *
	 * @param group
	 *            the group to be adopted
	 * @return a position in the receiver that contains all positions in
	 *         <code>group</code>, or <code>null</code> if none can be found
	 * @throws BadLocationException
	 *             if more than one position are affected by <code>group</code>
	 */
	 ...
	 *
	 * @param toFind
	 *            the linked position for which to find the closest position
	 ...
	 * positions are the actual positions and must not be modified; the array is
	 * a copy of internal structures.
	 *
