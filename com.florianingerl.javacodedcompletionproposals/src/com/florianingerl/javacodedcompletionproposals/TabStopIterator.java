...
/**
 * Iterator that leaps over the double occurrence of an element when switching
 * from forward to backward iteration that is shown by
...
class TabStopIterator {
	/**
	 * Comparator for <code>LinkedPosition</code>s. If the sequence number of
	 * two positions is equal, the offset is used.
	 */
	 ...
	 * <code>current</code> and sets <code>fIndex</code> accordingly. If
	 * <code>current</code> is in the iteration set, the next in turn is chosen.
	 *
	 * @param current
	 *            the current position
	 * @return <code>true</code> if there is a next position, <code>false</code>
	 *         otherwise
	 */
	 ...
	 * <code>current</code>. If <code>current</code> is in the iteration set,
	 * the previous in turn is chosen.
	 *
	 * @param current
	 *            the current position
