...
import org.eclipse.jdt.internal.ui.preferences.CodeTemplateSourceViewerConfiguration;
...
public class EditJavaCodedTemplateDialog extends StatusDialog {
	...
	protected void okPressed() {
		...
		try {
			...
			status.setError("Not a valid template pattern!\r\n" + e.getLocalizedMessage());
			updateStatus(status);
			return;
		}
		fTemplate = template;
	...
	private void updateStatusAndButtons() {
		...
		if (!fSuppressError && isEmpty) {
			...
			status = new StatusInfo();
			status.setError("The name of a template must also be a valid Java class name!");
		} else if (fNameText != null && isAlreadyNameOfOtherTemplate(fNameText.getText())) {
			status = new StatusInfo();
			status.setError(fNameText.getText() + " is already the name of another template!");
		}
		updateStatus(status);
	}

	private static final Pattern pJavaClassName = Pattern.compile("[a-zA-Z_$][a-zA-Z\\d_$]*");
	...
	private boolean isValidTemplateName(String name) {
		return pJavaClassName.matcher(name).matches();
	}

	private boolean isAlreadyNameOfOtherTemplate(String name) {
		if (fTemplate != null && name.equals(fTemplate.getName()))
			return false;
		return Stream.of(JavaCodedTemplatePlugin.getDefault().getTemplateStore().getTemplates()).map(Template::getName)
